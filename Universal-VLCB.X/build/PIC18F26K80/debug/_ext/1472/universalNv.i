...
static const uint8_t channelDefaultNVs[11][7] = {
    {0,6,4,4,0,0,0},
    {1,6,0,0,0,0,0},
    {2,6,128,128,235,235,0},
    {3,6,128,127,64,3,60},
    {4,6,3,128,128,128,0},
    {5,6,128,16,0,0,0},
    {6,6,0,123,32,7,255},
...
uint8_t APP_nvDefault(uint8_t index) {
    ...
    return channelDefaultNVs[(((uint8_t)((index-16)/7)) < 8) ? 0 : 0][(index-16) % 7];
}
...
# 230 "../universalNv.c"
            case 2:
                ...
                if (index == (16 + 7*(io) + 2)) {
                    setServoState(io, 2);
                    startServoOutput(io, 2);
                } else if (index == (16 + 7*(io) + 3)) {
                    setServoState(io, 1);
                    startServoOutput(io, 1);
                } else if (index == (16 + 7*(io) + 1)) {
                    if (((value & 0x80) == 0x80) &&
                            (oldValue & 0x80) == 0) {


                        setServoPosition(io, (uint8_t)(getNV((16 + 7*(io) + 3))/2 + getNV((16 + 7*(io) + 2))/2));
                    }
                }
                break;
            ...
            case 3:
                if (index == (16 + 7*(io) + 3)) {
                    setBounceState(io, 2);
                    startBounceOutput(io, 2);
                } else if (index == (16 + 7*(io) + 2)) {
                    setBounceState(io, 1);
                    startBounceOutput(io, 1);
                } else if (index == (16 + 7*(io) + 1)) {
                    if (((value & 0x80) == 0x80) &&
                            (oldValue & 0x80) == 0) {


                        currentPos[io] = (uint8_t)(getNV((16 + 7*(io) + 3))/2 + getNV((16 + 7*(io) + 2))/2);
                    }
                }
                break;
            ...
            case 4:
                if (index == (16 + 7*(io) + 3)) {
                    setMultiState(io, 0);
                    startMultiOutput(io, 0);
                } else if (index == (16 + 7*(io) + 4)) {
                    setMultiState(io, 1);
                    startMultiOutput(io, 1);
                } else if (index == (16 + 7*(io) + 5)) {
                    setMultiState(io, 2);
                    startMultiOutput(io, 2);
                } else if (index == (16 + 7*(io) + 6)) {
                    setMultiState(io, 3);
                    startMultiOutput(io, 3);
                } else if (index == (16 + 7*(io) + 1)) {
                    if (((value & 0x80) == 0x80) &&
                            ((oldValue & 0x80) == 0)) {
...
void defaultNVs(uint8_t io, uint8_t type) {
    ...
    for (i=1; i<7; i++) {
        saveNV((16 + 7*(io) + 0)+i , channelDefaultNVs[type][i]);
    }
    loadNvCache();
}
