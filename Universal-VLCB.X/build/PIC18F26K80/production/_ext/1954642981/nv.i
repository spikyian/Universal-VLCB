...
static void nvFactoryReset(void) {
    uint8_t i;
    for (i=1; i<= 127; i++) {
        writeNVM(FLASH_NVM_TYPE, 0xFF80 +i, APP_nvDefault(i));
    }
}
...
void loadNvCache(void) {
    ...
    for (i=1; i<= 127; i++) {
        temp = readNVM(FLASH_NVM_TYPE, 0xFF80 +i);
        if (temp < 0) {
...
# 184 "../../VLCBlib_PIC/nv.c"
void saveNV(uint8_t index, uint8_t value) {

    nvCache[index] = value;

    writeNVM(FLASH_NVM_TYPE, 0xFF80 +index, value);
}
# 203 "../../VLCBlib_PIC/nv.c"
