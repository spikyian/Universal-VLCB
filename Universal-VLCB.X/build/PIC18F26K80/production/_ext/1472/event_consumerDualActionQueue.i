# 1 "../event_consumerDualActionQueue.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 288 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "../event_consumerDualActionQueue.c" 2
# 42 "../event_consumerDualActionQueue.c"
...
static Processed consumer2QProcessMessage(Message *m) {
    ...
    if ( ! (opc&0b00000001)) {
        ...
        for (e=20 -1; e>=1 ;e--) {
            ...
            if (e > 1) {
                nextAction = evs[e-1];
                nextSimultaneous = nextAction & 0x80;
            } else {
...
Boolean pushTwoAction(ActionAndState a) {
    if (expedited) {
        ...
        if (expeditedWriteIndex >= 8) expeditedWriteIndex = 0;
        return TRUE;
    }
    if (((normalWriteIndex+1)&(64 -1)) == normalReadIndex)
        return FALSE;
    normalQueueBuf[normalWriteIndex++] = a;
    if (normalWriteIndex >= 64) normalWriteIndex = 0;
    return TRUE;
}
...
void deleteTwoActionQueue(uint8_t index) {
    ...
    qty = (normalWriteIndex - normalReadIndex) & (64 -1);
    if (index < qty) {
        if (normalReadIndex == normalWriteIndex) return;
        index += normalReadIndex;
        if (index >= 64) {
            index -= 64;
        }
