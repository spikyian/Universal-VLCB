...
void initInputScan(void) {
    for (io=0; io<16; io++) {
        ...
        if (getNV((16 + 7*(io) + 1)) & 0x10) {
            outputState[io] = (uint8_t)readNVM(EEPROM_NVM_TYPE, ((eeprom_address_t)((0x3FF -8))-25)+io);
        }
        delayCount[io] = 0;
    }
}
...
void inputScan(void) {
    for (io=0; io< 16; io++) {
        if (getNV((16 + 7*(io) + 0)) == 0) {
            uint8_t input = readInput(io);
            if (input != inputState[io]) {
                Boolean change = FALSE;

                if (inputState[io] && (delayCount[io] == getNV((16 + 7*(io) + 2)))) {
                    change = TRUE;
                }
                if (!inputState[io] && (delayCount[io] == getNV((16 + 7*(io) + 3)))) {
                    change = TRUE;
                }
                if (change) {
                    ...
                    if (getNV((16 + 7*(io) + 1)) & 0x10) {
                        ...
                        writeNVM(EEPROM_NVM_TYPE, ((eeprom_address_t)((0x3FF -8))-25)+io, outputState[io]);
                     } else {
                        outputState[io] = input;
                    }
                    ...
                    if (getNV((16 + 7*(io) + 1)) & 0x08) {
                        if (outputState[io]) {
                            ...
                            if (getNV((16 + 7*(io) + 1)) & 0x40) {
                                sendProducedEvent(((8 + 4*(io))+0), EVENT_OFF);
                            } else {
                                sendProducedEvent(((8 + 4*(io))+0), EVENT_ON);
                            }
                        } else {
                            if (getNV((16 + 7*(io) + 1)) & 0x40) {
                                sendProducedEvent(((8 + 4*(io))+1), EVENT_OFF);
                            } else {
                                sendProducedEvent(((8 + 4*(io))+1), EVENT_ON);
                            }
                        }
                    } else {

                        if (getNV((16 + 7*(io) + 1)) & 0x40) {
                            sendProducedEvent(((8 + 4*(io))+0), !outputState[io]);
                        } else {
                            sendProducedEvent(((8 + 4*(io))+0), outputState[io]);
                        }
                    }
