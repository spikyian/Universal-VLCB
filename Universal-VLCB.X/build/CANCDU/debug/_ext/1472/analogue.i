...
extern const Config configs[14];
# 37 "../analogue.c" 2
...
AnalogueStates analogueState[14];
...
void initAnaloguePort(unsigned char io) {
    analogueState[io].eventState = 0;
    analogueState[io].portState = 0;
}
...
void pollAnalogue(void) {
    ...
    if ((getNV((16 + 7*(portInProgress) + 0)) == 5) || (getNV((16 + 7*(portInProgress) + 0)) == 6)) {
        ...
        if (haveRequestedAdc) {
            ...
            if ((setupState == 0) || (portInProgress != setupIo)) {
                if (getNV((16 + 7*(portInProgress) + 0)) == 6) {


                    lthreshold = getNV((16 + 7*(portInProgress) + 5));
                    lthreshold = lthreshold << 8;
                    lthreshold |= getNV((16 + 7*(portInProgress) + 6));
                    hthreshold = lthreshold;
                    lthreshold -= getNV((16 + 7*(portInProgress) + 3));
                    hthreshold += getNV((16 + 7*(portInProgress) + 3));
                    lhysteresis = lthreshold + getNV((16 + 7*(portInProgress) + 4));
                    hhysteresis = hthreshold - getNV((16 + 7*(portInProgress) + 4));

                    if ((analogueState[portInProgress].eventState != 2) && (adc >= hthreshold)) {

                        if (analogueState[portInProgress].portState == 1) {
                            sendProducedEvent(((8 + 4*(portInProgress))+1), !(getNV((16 + 7*(portInProgress) + 1) ) & 0x40));
                        }
                        analogueState[portInProgress].eventState = 2;
                    } else if ((analogueState[portInProgress].eventState == 2) && (adc <= hhysteresis)) {

                        if (analogueState[portInProgress].portState == 1) {
                            if (!(getNV((16 + 7*(portInProgress) + 1) ) & 0x08)) {
                                sendProducedEvent(((8 + 4*(portInProgress))+1), getNV((16 + 7*(portInProgress) + 1) ) & 0x40);
                            }
                        }
                        analogueState[portInProgress].eventState = 0;
                    }
                } else {
                ...
                if ((analogueState[portInProgress].eventState != 1) && (adc <= lthreshold)) {

                    if (analogueState[portInProgress].portState == 1) {
                        sendProducedEvent(((8 + 4*(portInProgress))+0), !(getNV((16 + 7*(portInProgress) + 1) ) & 0x40));
                    }
                    analogueState[portInProgress].eventState = 1;
                } else if ((analogueState[portInProgress].eventState == 1) && (adc >= lhysteresis)) {
                ...
                if (setupState == 2) {
                    ...
                    saveNV((16 + 7*(portInProgress) + 5), adc&0xff);
                    saveNV((16 + 7*(portInProgress) + 6), adc>>8);
                }
                setupState = 0;
            }
            analogueState[portInProgress].portState = 1;
        }
    }

    portInProgress++;
    if (portInProgress >= 14) {
        portInProgress = 0;
    }
